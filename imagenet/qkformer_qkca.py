# QKFormer variant: QKTA (stage1+2) + QKCA (stage3) — no SSA, no Value projection
# Based on paper Table 6 ablation: QKTA+QKCA achieves 81.04% on CIFAR-100 vs 81.15% baseline (-0.11%)
# from visualizer import get_local

# Explanations for uninitiated readers (EFUR):
# Most important imports are torch (ofc),spikingjelly and timm. 
# Spikingjelly is providing us with a ready to use LIF model.
# timm is a PyTorchvision model library. In this context used for weight normalization 
import torch
...
# EFUR 
# Apparently a function that the orignial authors used to print the sparcity of a tensor 
...
# EFUR
# This is the Spiking MLP Block. A two layer feedforward network with the following structure per layer:
# Conv2d(1x1) -> Batch Normalization (BN) -> LIF 
class MLP(nn.Module):
...
# EFUR 
# This block implements the QK-Token-Attention (QKTA). Two paths are initialized one for Q and K
# respectively. 
#  
class Token_QK_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Row summation -> token attention vector (eq. 6 first part)
        q = torch.sum(q, dim = 3, keepdim = True)
        attn = self.attn_lif(q)

        # column mask on K (eq. 6 second part)
        x = torch.mul(attn, k)

        # output projection using LIF (eq. 7)
        x = x.flatten(2, 3)
...
# EUFR
# Implementation for QKCA
class QK_Channel_Attention(nn.Module):
    """Channel attention via column summation over tokens (QKCA).

    QKTA sums Q over channels → [N,1] token mask applied to K columns.
    QKCA sums Q over tokens  → [1,D] channel mask applied to K rows.
    No Value projection — eliminates the V term entirely.

    Paper formula (Section 3.2):
        Ac = SN( sum_{j=0}^{N} Q_{i,j} )   # [1, D] per-channel attention
        X' = Ac ⊗ K                          # broadcast over token dimension
    """
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
    ...
    def forward(self, x):
        ...
        # Column summation over tokens → [T, B, num_heads, C//num_heads, 1] channel attention vector
        q = torch.sum(q, dim=4, keepdim=True)
        attn = self.attn_lif(q)
        x = torch.mul(attn, k)  # broadcast [T,B,h,C/h,1] over [T,B,h,C/h,N]
...
class ChannelSpikingTransformer(nn.Module):
    """Block using QK_Channel_Attention (QKCA) — no Value projection."""
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.cssa = QK_Channel_Attention(dim, num_heads)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features=dim, hidden_features=mlp_hidden_dim, drop=drop)
...
class hierarchical_spiking_transformer(nn.Module):
    def __init__(self,
        ...
        # Stage 3 uses ChannelSpikingTransformer (QKCA) instead of SpikingTransformer (SSA)
        stage3 = nn.ModuleList([ChannelSpikingTransformer(
...
def QKFormer_10_384_QKCA(T=1, **kwargs):
    """QKTA (stage1+2) + QKCA (stage3) variant — no SSA, no Value projection."""
...
def QKFormer_10_512_QKCA(T=1, **kwargs):
    model = hierarchical_spiking_transformer(
        T=T,
        img_size_h=224, img_size_w=224,
...
def QKFormer_10_768_QKCA(T=1, **kwargs):
    model = hierarchical_spiking_transformer(
        T=T,
        img_size_h=224, img_size_w=224,
...
if __name__ == '__main__':
    ...
    model = QKFormer_10_384_QKCA(T=4).cuda()
